...
class ExchangeClient {
    ...
    public static function getAdditionalLogs($messageId, $maxResults, $sender = NULL, $subject = NULL, $utcMilliseconds = 0) {
        if(is_null($maxResults) || !is_numeric($maxResults)) {
            $maxResults = 20;
        }
        ...
        if(empty($messageId)) {
            $query .= "WHERE logmain.message_id is null \n";
            $query .= "AND (" . "logmain.date_time BETWEEN \"" . date_format($startDttm, "Y-m-d H:i:s") . "\" AND \"" . date_format($endDttm, "Y-m-d H:i:s") . "\") \n";

            if(empty($sender)) {
                $query .= "AND logmain.sender_address is null \n";
            } else {
                $query .= "AND \"" . $sender . "\" = logmain.sender_address \n";
            }
            
            if(empty($subject)) {
                $query .= "AND logmain.message_subject is null \n";
            } else {
                $query .= "AND \"" . $subject . "\" = logmain.message_subject \n";
            }
        } else {
            //if we have a messageId, that's the only thing we need to search off of
            $query .= "WHERE \"" . $messageId . "\" = logmain.message_id \n";
        }

        $query .= "ORDER BY date_time \n";
        $query .= "LIMIT " . $maxResults;

        //return ExchangeClient::exchangeError("Query: " . htmlentities($query));
